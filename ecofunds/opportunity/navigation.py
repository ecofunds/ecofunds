...
from ecofunds.core.models import FundingOpportunity

def get_nodes(request):
    res = []

    items = FundingOpportunity.published.all()
    ...
    for item in items:
        ...
        if not date.year in years_done:
            ...
            year_node = NavigationNode(date.year, reverse('opportunity_year', kwargs=dict(year=date.year)))
            year_node.childrens = []
            months_done = []
        ...
        if not date.month in months_done:
            ...
            month_node = NavigationNode(datetime.strftime(date, '%B'),
                                reverse('opportunity_month', kwargs=dict(
                                    year=date.year,
        ...
        if not date.day in days_done:
            ...
            day_node = NavigationNode(datetime.strftime(date, '%d'),
                                reverse('opportunity_day', kwargs=dict(
                                    year=date.year,
                                    month=datetime.strftime(date, '%m'),
